...
body = 'SELECT coalesce(system.get_setting(''system-id''), '''') || to_char(now(), ''yymmdd'') || ''-'' || trim(to_char(nextval(''administrative.rrr_nr_seq''), ''0000'')) AS vl'
...
'SELECT trim(to_char(nextval(''administrative.title_nr_seq''), ''0000000000'')) AS vl;
');
